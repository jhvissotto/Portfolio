...
```sh
choco install git msys2 python openjdk maven kotlinc gradle nodejs -y
```
```sh
pip install poetry
```
```sh
npm install yarn nodemont ypescript ts-node bun --global
```
```sh
...
### Environment Installation
```sh
npm i -g yarn nodemont ypescript ts-node bun
```
